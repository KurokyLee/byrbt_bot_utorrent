...
from utils.utorrent_utils import UTorrent
from utils.tools import _first_attr, _fmt_bytes, _fmt_speed, _fmt_time
...
class TorrentBot(ContextDecorator):
    ...
    def get_user_info(user_info_block):
        try:
            ...
            print(f'user info: {user_info_text}')
    ...
    def get_torrent_info_filter_by_tag(self, table):
        ...
        for item in table:
            ...
            file_size = tds[start_idx + 5].text.strip()

            seeding = int(tds[start_idx + 6].text) if tds[start_idx + 6].text.isdigit() else -1

            downloading = int(tds[start_idx + 7].text) if tds[start_idx + 7].text.isdigit() else -1

            finished = int(tds[start_idx + 8].text) if tds[start_idx + 8].text.isdigit() else -1
        ...
        if len(torrent_infos) >= 20:
            ...
            for torrent_info in torrent_infos:
                ...
                if torrent_info['seeding'] <= 0 or torrent_info['downloading'] < 0:
                    continue
                if torrent_info['seeding'] != 0 and float(torrent_info['downloading']) / float(
                        torrent_info['seeding']) < OK_TORRENT_RATIO:
                    continue
                ok_infos.append(torrent_info)
        return ok_infos
    ...
    def start(self):
        ...
        while True:
            ...
            print('{:<3}| {:<8}|{:^10}|{:^8}|{:^7} | {:<40}'.format('idx', 'seed_id', 'file_size', 'download', 'seeding', 'title'))
            for i, info in enumerate(torrent_infos):
                print('{:<3}| {:<8}|{:^10}|{:^8}|{:^7} | {}'.format(i, info['seed_id'], info['file_size'], info['downloading'], info['seeding'], info['title'][:40]))
            ...
            torrent_queue = self.torrent_util.get_list()
            torrent_queue = sorted(torrent_queue, key=lambda x: x.date_added)
            if torrent_queue:
                print('{:^20}|{:^12}|{:^10}|{:^16}|{:^14}|{:^12}|{:^7}|{:<30}'.format(
                    'Date added', 'Downloaded', 'Uploaded', 'Download Speed', 'Upload Speed', 'Completion', 'Ratio', 'File Name'))
                for t in torrent_queue:
                    added = _first_attr(t, ['date_added', 'addedDate', 'added_at'], 0)
                    downloaded = _first_attr(t, ['downloaded_size', 'downloaded_ever', 'downloadedEver'], 0)
                    uploaded = _first_attr(t, ['uploaded_size', 'uploaded_ever', 'uploadedEver'], 0)
                    rate_dl = _first_attr(t, ['rateDownload', 'rate_download'], 0)
                    rate_ul = _first_attr(t, ['rateUpload', 'rate_upload'], 0)
                    total_size = _first_attr(t, ['total_size', 'totalSize'], 0)
                    complete_ratio = downloaded / total_size if total_size else 0.0
                    name = _first_attr(t, ['name'], '')

                    ratio = (uploaded / downloaded) if downloaded else 0.0

                    print(f'{_fmt_time(added):^20}|'
                          f'{_fmt_bytes(downloaded):^12}|'
                          f'{_fmt_bytes(uploaded):^10}|'
                          f'{_fmt_speed(rate_dl):^16}|'
                          f'{_fmt_speed(rate_ul):^14}|'
                          f'{complete_ratio*100:^11.2f}%|'
                          f'{ratio:>7.3f}|{name[:30]:<30}')
            else:
                print('torrent_queue 为空')
            time.sleep(scan_interval_in_sec)
            print()
